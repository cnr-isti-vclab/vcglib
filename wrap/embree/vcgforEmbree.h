...
namespace vcgEmb{
    ...
    public:
        ...
        static void naiveEmbreeShadows(MeshType &m, Point3f rayDirection, bool showHit, bool verbose){
    ...
    public:
        template <class MeshType>
        static std::tuple<RTCScene,RTCGeometry,RTCDevice> loadVCGMeshInScene(MeshType &m){
            ...
            return std::make_tuple(scene, geometry, device);
        }

    public:
        template <class MeshType>
        static void embreeAmbientOcclusion(MeshType &inputM, int nRay, int threads){
            auto tup = loadVCGMeshInScene(inputM);
            RTCScene scene = get<0>(tup);
            RTCGeometry geometry =std::get<1>(tup);
            RTCDevice device = std::get<2>(tup);
            RTCRayHit rayhit;
            ...
            #pragma omp parallel shared(inputM) private(i) num_threads(threads)
            {
    ...
    public:
        template <class MeshType>
        static void embreeAmbientOcclusion(MeshType &inputM, int nRay, std::vector<Point3f> unifDirVec, int threads){
            auto tup = loadVCGMeshInScene(inputM);
            RTCScene scene = get<0>(tup);
            RTCGeometry geometry =std::get<1>(tup);
            RTCDevice device = std::get<2>(tup);
    ...
    public:
        template <class MeshType>
        static void embreeObscuranceOcclusion(MeshType &inputM, int nRay, float tau, int threads){
            auto tup = loadVCGMeshInScene(inputM);
            RTCScene scene = get<0>(tup);
            RTCGeometry geometry =std::get<1>(tup);
            RTCDevice device = std::get<2>(tup);
    ...
    public:
        template <class MeshType>
        static void embreeObscuranceOcclusion(MeshType &inputM, int nRay, std::vector<Point3f> unifDirVec, float tau, int threads){
            auto tup = loadVCGMeshInScene(inputM);
            RTCScene scene = get<0>(tup);
            RTCGeometry geometry =std::get<1>(tup);
            RTCDevice device = std::get<2>(tup);
    ...
    public:
        template <class MeshType>
        static void embreeSDF(MeshType &inputM, int nRay){
            auto tup = loadVCGMeshInScene(inputM);
            RTCScene scene = get<0>(tup);
            RTCGeometry geometry =std::get<1>(tup);
            RTCDevice device = std::get<2>(tup);
    ...
    public:
        template <class MeshType>
        static std::vector<Point3f> AOBentNormal(MeshType &inputM, int nRay, int threads){
            auto tup = loadVCGMeshInScene(inputM);
            RTCScene scene = get<0>(tup);
            RTCGeometry geometry =std::get<1>(tup);
            RTCDevice device = std::get<2>(tup);
            ...
            #pragma omp parallel shared(inputM) private(i) num_threads(threads)
            {
                ...
                //bentNormalDir[0]/=accRays;
                //bentNormalDir[1]/=accRays;
                //bentNormalDir[2]/=accRays;
