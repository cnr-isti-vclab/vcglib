...
static void RelativeName(char * OutFileName, const char * NamePattern, const char * OrigName)
{
    ...
    const char * Subst = strstr(NamePattern, "&i");
    if (Subst){
...
//PB line changed, it was: if (!(DoModify || DoReadAction) || ShowTags){
...
#if 0 //disabled because it is never called
//--------------------------------------------------------------------------
