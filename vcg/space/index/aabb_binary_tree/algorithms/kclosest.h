...
#ifndef __VCGLIB_AABBBINARYTREEKCLOSEST
#define __VCGLIB_AABBBINARYTREEKCLOSEST
...
protected:
	...
	static void DepthFirstCollect(NodeType * node, OBJPOINTDISTANCEFUNCT & getPointDistance, const unsigned int k, const CoordType & p, ScalarType & mindmax, PQueueType & pq) {
		...
		if (pq.size() >= k) {
			const ScalarType dmin = LowerClamp(dc - node->boxHalfDims, (ScalarType)0).SquaredNorm();
			if (dmin >= mindmax) {
				return;
			}
		}
...
#endif // #ifndef __VCGLIB_AABBBINARYTREEKCLOSEST
