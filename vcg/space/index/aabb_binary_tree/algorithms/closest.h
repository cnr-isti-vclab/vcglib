...
#ifndef __VCGLIB_AABBBINARYTREECLOSEST
#define __VCGLIB_AABBBINARYTREECLOSEST
...
#include <vcg/space/index/aabb_binary_tree/algorithms/shared/shared_utils.h>
	...
	static inline ObjPtr Closest(TreeType & tree, OBJPOINTDISTANCEFUNCT & getPointDistance, const CoordType & p, const ScalarType & maxDist, ScalarType & minDist, CoordType & q) {
		...
		while (!candidates->empty()) {
			...
			for (NodePtrVector_ci bv=candidates->begin(); bv!=candidates->end(); ++bv) {
				...
				(*bv)->ScalarValue() = LowerClamp(dc - (*bv)->boxHalfDims, (ScalarType)0).SquaredNorm();
...
#endif // #ifndef __VCGLIB_AABBBINARYTREECLOSEST
